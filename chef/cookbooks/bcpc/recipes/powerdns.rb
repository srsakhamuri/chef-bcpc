...
include_recipe 'bcpc::powerdns-apt'
...
mgmt_cidr = node['bcpc']['management']['aggregate']['cidr']
management_zone = calc_reverse_dns_zone(mgmt_cidr)

management_zone.each do |zone|

  execute "add management #{zone} reversed zone" do
    environment ({'MYSQL_PWD' => mysql_root_password})
    db = node['bcpc']['dbname']['pdns']
    ...
    not_if "mysql -u root -e 'select name from pdns.domains' | grep -w #{zone}"
...
template dns_records_file do
  source "powerdns_generate_dns_records.sql.erb"
  owner "root"
  group "root"
  mode 00644
    ...
    lazy {
      {
        ...
        :pdns_anycast_ip     => node['bcpc']['anycast']['pdns']['ip'],
        ...
        :rev_guest_network_zones => rev_guest_network_zones
      }
    }
  )
